...
每轮写入时，在 `current_state.md` 中记录强执行变量的场景瞬时值与长期基线偏差。

**仅允许以下 8 个强执行变量，禁止使用任何其他变量名：**
1. 信任度
2. 依赖感
3. 自主性
4. 主动性
...
**弱执行变量（仅限以下 4 个，禁止使用任何其他变量名）：**
- 恢复速度
- 撒娇倾向
- 嘴硬程度
- 吃醋倾向

弱执行变量仅在显著波动或触发制衡规则时才记录，不强制每轮全量标记。
**禁止自创任何不在上述列表中的变量名**（如"亲密度""好感度""攻略度"等）。

基线值必须从 `world/relationships.md` 中读取，不得自行估计。若基线读取成功，直接引用数值；若基线读取失败导致无法获取数值，标注"基线读取失败"，不得编造基线值，也不得写"基线待建立"。
...
- 不得在场景瞬时变量中使用 12 变量体系之外的变量名（禁止自创如"亲密度""好感度""攻略度"等）。
